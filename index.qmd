...
format:
  dashboard:
    nav-buttons: [github]
    github: https://github.com/emc-viroscience/bird-surveillance-dashboard
...
usuv_map <- base_map +
  geom_point(
    ...
    alpha = 0.75, shape = 21, color = "transparent", stroke = 0.4
  ) +
    geom_text(
    data = fa_pos_clustered_df,
    aes(
      x = Long_rep,
      y = Lat_rep,
      label = ifelse(cluster_size>=15,cluster_size,'')
    ),
    size = 3, color = "white"
  ) +
...
total_map <- base_map +
  geom_point(
    ...
    fill = "seagreen3", alpha = 0.6, shape = 21, color = "transparent", stroke = 0.4
  ) +
  geom_text(
    data = dead_birds_clustered,
    aes(
      x = Long_rep,
      y = Lat_rep,
      label = ifelse(cluster_size>=50,cluster_size,'')
    ),
    size = 3
  ) +
...
#| title: "Positive Dead Birds Locations, 2016-2022"
...
usuv_map <- base_map +
  geom_point(
    ...
    aes(
      x = Long_rep,
      y = Lat_rep,
      label = ifelse(cluster_size>=6,cluster_size,'')
    ),
    size = 3, color = "white"
  ) +
  scale_size_continuous(name = "Virus detections", range = c(1, 7.5)) +
...
p <- ggplot(
  ...
  aes(
    ...
    color = Virus,
    group = Virus,
    text = paste("Season:", year_season, "<br>Result:", Virus, "<br>Percentage:", round(percent, digits=2)))
  ) +
  geom_line() +
  labs(title = "Live Blackbirds and Song Thrushes", y = "", x = "Season") +
  scale_color_manual(values = c("USUV-Ab" = "firebrick", "WNV-Ab" = "orange", "Orthoflavivirus-Ab" = "#901fd1")) +
...
This dashboard is built with [Quarto Dashboards](https://quarto.org/docs/dashboards/). All code is open source and can be found here: [<i class="bi bi-github"></i> Source code](https://github.com/emc-viroscience/bird-surveillance-dashboard)
